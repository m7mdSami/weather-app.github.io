    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="list-group-item d-flex justify-content-between" v-bind:class="[booleanValue ? 'col-lg-6' : 'col-lg-12']" v-for="(i, index) in city.list" v-if="index < 4">
          {{i.dt_txt}} <span class="d-flex align-items-center"><h5>{{i.main.temp}}</h5> <h1>°C</h1></span>
        </div>
      </div>
    </div>
  </div>
...
<script>
    export default {
        ...
        methods: {
          fetchWeather(){
            this.$http.get('http://api.openweathermap.org/data/2.5/forecast?q='+this.selected+'&mode=json&units=metric&appid=5121f13fd90c7f33f764109bcfc0c9fd').then(function(res){
              this.city = res.body;
            });
          },
          toggle: function(e){
